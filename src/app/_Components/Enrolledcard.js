            ...
            try {
                ...
                if (responseVerify.success) {
                    const fetchCart = async () => {
                        const cartResponse = await fetch(`${appBaseUrl}/api/enrolledcourse`, {
                            method: "Post",
                            body:JSON.stringify({userDetails:responseVerify.verifytoken.userid})
                        });
              ...
              {cart.map((item, idx) => (
                <Link key={idx} href={`/courses/${item.courseId}/${item?.videosrc[0]?._id}`}  className='cursor-pointer' >
                  ...
                  <motion.div
                    ...
                    className="rounded-lg  overflow-hidden shadow-md border border-gray-200 dark:bg-gray-800 dark:border-gray-700 max-w-sm w-[150px] h-[210px] sm:w-[300px] sm:h-fit lg:w-[350px]"
                  >
                    ...
                    <div className='sm:p-5 p-3'>
                      <div className='flex flex-col gap-2'>
                        <div className='flex items-center justify-between'>
                          ...
                          <h5 className=" font-bold text-[#d42eeb] dark:text-white truncate sm:text-[18px] text-[8px] capitalize">{item?.courseName}</h5>
                        ...
                        <div className='flex justify-between items-center text-sm text-gray-600 dark:text-gray-400 gap-2'>
                          <div className='flex items-center gap-1'>
                            ...
                            <svg
                              className="w-4 h-4 text-[#882096] dark:text-white"
                              aria-hidden="true"
