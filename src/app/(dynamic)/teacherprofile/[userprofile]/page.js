...
<div class="w-8/12 md:w-7/12 ml-4">
  ...
  <div class="hidden md:block">
                      <h1 class="font-semibold capitalize">{items?.name} { items?.lastname}</h1>
                      <span className=' text-gray-500'>{ items?.profession}</span>
                      <p>{ items?.bio}</p>
  </div>
                    <div className='w-full flex gap-2 h-fit mt-4'>
                      
                      <Follow data={content.userId} teacherid={ items.userId} />
                      {items?.email ? <button  onClick={() => { sendEmail(items.email) }} className=' bg-[#deecdb] text-gray-600 w-full h-fit rounded-xl flex items-center justify-center p-1 gap-2'> <svg className="w-6 h-6 text-gray-600 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 0 0-2 2v4m5-6h8M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2m0 0h3a2 2 0 0 1 2 2v4m0 0v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-6m18 0s-4 2-9 2-9-2-9-2m9-2h.01" />
                      </svg><span className='text-gray-600 '>Email</span></button> : null}
</div>
</div>
                              ...
                              <div className='w-full shadow-md rounded-xl'>
                                <div id="defaultTabContent" className='bg-[#f8f0f0] p-7 rounded-3xl flex gap-9 items-center w-full'>
                                  ...
                                  <div className="rounded-lg dark:bg-gray-800" id="about" role="tabpanel" aria-labelledby="about-tab">
                                    ...
                                    <h2 className="mb-3 text-[1.2vw] font-semibold tracking-tight text-gray-900 dark:text-white">{items?.courseName}</h2>
                                    <p className="mb-3 text-gray-500 text-[0.8vw] dark:text-gray-400">{items?.courseDesc}</p>

                                    <div className='tag mt-3'><span className='pl-3 pr-3 p-1 text-[0.7vw] text-[#d42eeb] rounded-full font-semibold border-2 border-[#d42eeb]'>{items?.tags}</span></div>
                                    <div className='flex items-center gap-2'>
                        
                                      <span className="inline-flex text-[0.9vw] items-center mt-3 font-medium text-blue-600 hover:text-blue-800 dark:text-blue-500 dark:hover:text-blue-700">
                                        Learn more
